...
Try this query to find all books from 2020:

<InteractiveQueryEditor
  initialQuery='db.books.find({ year: 2020 })'
/>
...
<InteractiveQueryEditor
  initialQuery='db.books.find({ $or: [{ year: 2010 }, { pages: { $gt: 200 } }] })'
/>
...
<InteractiveQueryEditor
  initialQuery='db.books.find({ $and: [{ year: 2010 }, { pages: { $gt: 200 } }] })'
/>
...
<InteractiveQueryEditor
  initialQuery='db.books.find({ year: 2010, pages: { $gt: 200 } })'
/>
...
<InteractiveExercise
  title="Challenge 1: Find books by exact inventory"
  description="Find all books where totalInventory is exactly 5."
  initialQuery='db.books.find({ totalInventory: 5 })'
  hint="Use exact equality matching with totalInventory field"
  solution='db.books.find({ totalInventory: 5 })'
/>

<InteractiveExercise
  title="Challenge 2: Find books with many pages"
  description="Find all books with more than 300 pages."
  initialQuery='db.books.find({ pages: { $gt: 300 } })'
  hint="Use the $gt operator to find pages greater than 300"
  solution='db.books.find({ pages: { $gt: 300 } })'
/>

<InteractiveExercise
  title="Challenge 3: Complex filtering"
  description="Find books in the Science genre that are more than 300 pages long."
  initialQuery='db.books.find({ genres: "Science", pages: { $gt: 300 } })'
  hint="Use genres field (it's an array) and combine with pages filter"
  solution='db.books.find({ genres: "Science", pages: { $gt: 300 } })'
/>
